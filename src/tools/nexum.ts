...
export function registerNexumTools(server:McpServer, getService:()=>NexumService=getNexum):void {
  ...
  tool("nexum_get_system_state","Read cached system, signatures, anomalies, structures, connections and source-labeled Nexum/ESI presence.",
    {map_id:id,system_id_or_name:id},(s,a)=>s.systemState(a.map_id,a.system_id_or_name));
  tool("nexum_get_presence","Read current or retained 48h Nexum viewer and ESI location telemetry. ESI supplements bound characters with existing ESI location authorization; online status remains unknown. Current rows merge by character, retaining source observations, timestamps, stale flags and conflicts. Bound characters outside this map have in_map=false. Does not include Nexum's account-alt or fleet occupancy badges: those use browser-session-only APIs unavailable to these keys. Missing presence is not evidence of absence from a system. Historical results include a retained pre-since baseline. Never treat inferred location as an explicit user report.",
